...
export function SVGBlobs({
  ...
  useEffect(() => {
    ...
    const circles = svg.querySelectorAll("circle");
    const expandedRadius = dotScale === "tiny" ? 10 : dotScale === "small" ? 24 : baseRadius;
    ...
    if (isGenerating && !wasGeneratingRef.current) {
      circles.forEach((circle) => {
        ...
        const currentRadius = Number.parseFloat(circle.getAttribute("r") ?? String(baseRadius));
        const startRadius = Number.isFinite(currentRadius) ? currentRadius : baseRadius;

        animate.setAttribute("values", `${startRadius};${expandedRadius};${baseRadius}`);
    ...
    >
      <defs>
        ...
        <filter
          ...
          height="4340"
        >
          <feGaussianBlur in="SourceAlpha" stdDeviation="8.5" result="blur"></feGaussianBlur>
          <feComposite
            in="SourceAlpha"
            in2="blur"
            operator="arithmetic"
          ...
          ></feFlood>
          <feComposite in="color" in2="innerRim" operator="in" result="glow"></feComposite>
