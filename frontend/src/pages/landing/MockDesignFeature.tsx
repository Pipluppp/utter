...
const MOCK_DESIGN_PREVIEW_TEXT =
	...
	useEffect(() => {
		...
		schedule(() => {
			...
			const intervalMs = Math.max(
				1,
				Math.floor(DESIGN_TIMING[0].duration / totalChars),
			);
			let charIndex = 0;
		...
		schedule(() => {
			...
			const intervalMs = Math.max(
				1,
				Math.floor(DESIGN_TIMING[2].duration / totalChars),
			);
			let charIndex = 0;
			const intervalId = scheduleInterval(() => {
				charIndex++;
...
function MockWaveform() {
	...
	const bars = [
		0.2, 0.45, 0.7, 0.5, 0.85, 0.6, 0.4, 0.75, 0.9, 0.55, 0.35, 0.8,
		0.65, 0.45, 0.95, 0.5, 0.7, 0.4, 0.6, 0.85, 0.3, 0.55, 0.75, 0.5,
		0.15, 0.1, 0.15, 0.1, 0.2, 0.4, 0.65, 0.85, 0.55, 0.7, 0.9, 0.45,
		0.6, 0.8, 0.5, 0.35, 0.75, 0.55, 0.4, 0.65, 0.8, 0.5, 0.3, 0.6,
	];
	return (
				...
				<div aria-hidden="true" className="space-y-6 p-6">
					...
					<div>
						...
						<div className="w-full border border-border bg-background px-4 py-3 text-sm text-foreground shadow-elevated">
							{voiceName || (
								<span className="text-faint">Name your voice...</span>
							)}
						</div>
					</div>
					...
					<div>
						...
						<div className="mt-2 flex flex-wrap gap-2">
							{["Warm & steady", "Bright & fast", "Low & cinematic"].map(
								(label) => (
									<div
										key={label}
										className={cn(
											...
											presetPressed && label === "Warm & steady"
												? "scale-95"
												: "scale-100",
										)}
									>
