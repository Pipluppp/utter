...
VOICE_DESIGN_BASE = os.getenv(
    "VOICE_DESIGN_BASE",
    "https://duncab013--qwen3-tts-voice-design-voicedesignservice"
)
...
def run_verification() -> dict:
    ...
    for i, prompt in enumerate(VERIFICATION_PROMPTS):
        ...
        if design_result["success"]:
            results["summary"]["passed"] += 1
            results["summary"]["total_audio_bytes"] += design_result["audio_size_bytes"] or 0
            
            if is_cold_start:
                results["summary"]["cold_start_latency_ms"] = design_result["latency_ms"]
            else:
    ...
    first_successful = next(
        (d for d in results["voice_designs"] if d["success"]), None
    )
    if first_successful:
    ...
    if results["summary"]["avg_warm_latency_ms"]:
        print(f"  Avg warm latency: {results['summary']['avg_warm_latency_ms']/1000:.1f}s")
    ...
    for design in results["voice_designs"]:
        ...
        size = f"{design['audio_size_bytes']:,}B" if design["audio_size_bytes"] else "N/A"
