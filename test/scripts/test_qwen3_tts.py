...
Usage:
    cd test
    uv run --with requests python test_qwen3_tts.py                     # Test 1.7B SDPA (default)
    uv run --with requests python test_qwen3_tts.py --model 1.7B-FA2    # Test 1.7B Flash Attention 2
...
For latency comparison between SDPA and FA2, use compare_fa2_sdpa.py instead.
"""
...
MODEL_ENDPOINTS = {
    ...
    "1.7B-FA2": {
        ...
        "languages": "https://duncab013--qwen3-tts-voice-clone-fa2-qwen3ttsservice-languages.modal.run",
    },
    "0.6B": {
...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["1.7B", "1.7B-FA2", "0.6B"],
        default="1.7B",
        help="Model variant to test: 1.7B (SDPA), 1.7B-FA2 (Flash Attention 2), or 0.6B (default: 1.7B)",
    )
    parser.add_argument(
        "--text",
