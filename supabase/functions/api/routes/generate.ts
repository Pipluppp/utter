...
generateRoutes.post("/generate", async (c) => {
  ...
  if (text.length > 10000) return jsonDetail("Text cannot exceed 10000 characters", 400)
  ...
  const refText = (voice as { reference_transcript: string | null }).reference_transcript
  if (!refText) {
    return jsonDetail(
  ...
  const { data: audioBlob, error: downloadError } = await admin.storage
    .from("references")
    .download(refKey)
  if (downloadError || !audioBlob) return jsonDetail("Failed to download reference audio.", 500)
  ...
  if (genError || !generation) return jsonDetail("Failed to create generation.", 500)
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : "Failed to submit generation job."
    await admin
      .from("tasks")
