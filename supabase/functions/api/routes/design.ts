...
designRoutes.post("/voices/design/preview", async (c) => {
  ...
  if (text.length > 500) return jsonDetail("Preview text must be 500 characters or less.", 400)
  if (!instruct) return jsonDetail("Voice description is required.", 400)
  if (instruct.length > 500) return jsonDetail("Voice description must be 500 characters or less.", 400)
  ...
  if (error || !task) return jsonDetail("Failed to create task.", 500)
...
designRoutes.post("/voices/design", async (c) => {
  ...
  if (audio.size > 1_000_000) return jsonDetail("Audio file must be under 1MB.", 400)

  const voiceId = crypto.randomUUID()
  const objectKey = `${userId}/${voiceId}/reference.wav`

  const admin = createAdminClient()
  const upload = await admin.storage.from("references").upload(objectKey, audio, {
